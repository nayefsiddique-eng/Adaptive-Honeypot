...
async def run_closed_loop_simulation(db: Session, session_count: int = 5, step_delay: float = 0.02) -> int:
    ...
    for i in range(session_count):
        ...
        # Trigger Digital Twin simulation & session Q-learning updates
            ...
            for session in active_sessions:
                ...
                if session.rl_state and session.rl_action:
                    try:
                        ...
                        logger.error(f"[DEMO] Error updating Q-table for session {session.session_id}: {q_err}")
                
                # Pre-generate explanation report
                try:
                    exp_engine = DeceptionExplanationEngine(db)
                    exp_engine.generate_soc_markdown_report(session.session_id)
                except Exception as exp_err:
                    logger.error(f"[DEMO] Error generating explanation for session {session.session_id}: {exp_err}")

            db.commit()

            # Execute Digital Twin Persona verification
            twin = DeceptionDigitalTwin(db)
            twin.simulate_adversary_session(random.choice(["script_kiddie", "red_team", "botnet"]))
        except Exception as reap_err:
            db.rollback()
            logger.error(f"[DEMO] Error during session closing & twin execution: {reap_err}")
