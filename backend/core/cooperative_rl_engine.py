...
def update_q_table_for_session(db: Session, session_id: str, state_str: str, action_str: str, deception_score: float) -> None:
    ...
    svc_act = action_str.split(":")[0] if ":" in action_str else action_str
    net_raw = session.rl_network_action or "medium"
    net_act = net_raw.split(":")[-1] if ":" in net_raw else net_raw
