...
def calculate_deception_score(interaction_level: str, fake_services: list, decoy_files: list, response_delay_ms: float) -> float:
    interaction_level_map = {"low": 0, "medium": 1, "high": 2, "deep": 3}
    score = (
        (interaction_level_map.get(interaction_level, 0) / 3.0) * 0.40 +
        (len(fake_services) / 5.0) * 0.20 +
        (len(decoy_files) / 6.0) * 0.20 +
        (min(response_delay_ms, 2000) / 2000.0) * 0.20
    )
...
def decide_behavior(
    ...
    response["deception_score"] = calculate_deception_score(
        response["interaction_level"],
        response["fake_services"],
        response["decoy_files"],
        response["response_delay_ms"]
    )
