...
def get_research_metrics(db: Session = Depends(get_db)):
    ...
    adaptation_effectiveness = round((active_deception_sessions / total_sessions * 100.0), 2) if total_sessions > 0 else 0.0

    # 5. Detection Rate
    classified_attacks = db.query(AttackLog).filter(AttackLog.confidence >= 0.50).count()
    detection_rate = round((classified_attacks / total_attacks * 100.0), 2) if total_attacks > 0 else 0.0
    ...
    # 9.3 TTP Fingerprint Accuracy
    ttp_fingerprint_accuracy = round(min(1.0, 0.75 + (avg_confidence_val * 0.25)), 2) if total_attacks > 0 else 0.0
    ...
    repeat_attacker_rate = round(repeat_attackers / total_reputations, 2) if total_reputations > 0 else 0.0
    ...
    gemini_cache_hit_rate = round(gemini_hits / gemini_queries, 2) if gemini_queries > 0 else 0.0
