...
│                                                          │
│  ┌────────────────────────────────────────────────────┐  │
│  │  FastAPI :8000  (binds to 127.0.0.1 only)         │  │
│  │  ├── Feature Extractor                            │  │
│  │  ├── ML Ensemble  (RF + XGBoost + IsoForest)      │  │
│  │  ├── Cooperative RL (Network / Service / Intel)   │  │
│  │  ├── SQLite Database                              │  │
│  │  └── Cyber-HUD Frontend                          │  │
│  └────────────────────────────────────────────────────┘  │
│                         ▲                                │
│            Authenticated analyst access only             │
└──────────────────────────────────────────────────────────┘
```
...
> **Honest note:** These numbers are on synthetic training data generated by `ml/train_classifier.py`. Performance on live traffic will differ — the data distribution may not capture all real-world attacker variability. The Isolation Forest is used as a second-opinion signal alongside the supervised classifier, not as a standalone decision gate.
